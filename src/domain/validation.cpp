...
bool ValidationReport::valid() const noexcept { return error_count() == 0; }

std::size_t ValidationReport::error_count() const noexcept {
    return static_cast<std::size_t>(std::count_if(diagnostics_.begin(), diagnostics_.end(), [](const auto& item) {
...
const std::vector<Diagnostic>& ValidationReport::diagnostics() const noexcept { return diagnostics_; }
