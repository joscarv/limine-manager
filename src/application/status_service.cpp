...
StatusReport StatusService::build(const model::SystemModel &model,
    ...
    const bool encryption_metadata_incomplete =
        system.root_encrypted &&
        (system.encrypted_backing_device.empty() || system.luks_uuid.empty());
    const bool large_snapshot_menu = model.snapshots.selected.size() > 100;
    const bool degraded =
        validation.valid() && (encryption_metadata_incomplete || large_snapshot_menu);
    ...
    out << "limine-manager status\n\n"
        ...
        << "  Root device: "
        << path_text(system.root_source.substr(0, system.root_source.find('['))) << '\n'
    ...
    if (!validation.valid() || encryption_metadata_incomplete || large_snapshot_menu) {
        ...
        if (encryption_metadata_incomplete)
            out << "  [WARNING] encryption.discovery: Encrypted root detected, but backing device "
                   "metadata is incomplete.\n";
        if (large_snapshot_menu)
            out << "  [WARNING] snapshots.menu: More than 100 selected snapshots will produce a "
                   "large Limine menu.\n";
