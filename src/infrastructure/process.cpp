...
ProcessResult PosixProcessRunner::run(const std::vector<std::string>& arguments) const {
    if (arguments.empty()) throw std::invalid_argument("Process argument list is empty");

    int pipe_fd[2]{};
    if (::pipe(pipe_fd) == -1) throw std::runtime_error("pipe failed: " + std::string(std::strerror(errno)));
    ...
    if (pid == 0) {
        ::close(pipe_fd[0]);
        if (::dup2(pipe_fd[1], STDOUT_FILENO) == -1 || ::dup2(pipe_fd[1], STDERR_FILENO) == -1) _exit(126);
        ::close(pipe_fd[1]);
    ...
    const int exit_code = WIFEXITED(status) ? WEXITSTATUS(status) : 128 + (WIFSIGNALED(status) ? WTERMSIG(status) : 0);
