...
std::string os_pretty_name(const FileSystem& filesystem) {
    ...
    while (std::getline(input, line)) {
        if (line.rfind("PRETTY_NAME=", 0) == 0) return unquote(line.substr(12));
    }
    return "Linux";
}

std::string require_command(const ProcessRunner& runner, std::vector<std::string> args, const std::string& label) {
    auto result = runner.run(args);
    if (result.exit_code != 0) throw std::runtime_error(label + " failed: " + trim(result.output));
    return trim(result.output);
}

std::string mount_field(const ProcessRunner& runner, const std::filesystem::path& target, const std::string& field) {
    return require_command(runner, {"findmnt", "--noheadings", "--raw", "--target", target.string(),
                                    "--output", field}, "findmnt " + target.string() + " " + field);
}
...
std::string option_value(const std::string& options, const std::string& key) {
    ...
    while (begin <= options.size()) {
        ...
        const auto item = options.substr(begin, end == std::string::npos ? std::string::npos : end - begin);
...
SystemInfo SystemDetector::detect() const {
    ...
    info.kernels = KernelDiscovery(filesystem_, {profile_.boot_mount, profile_.modules_root, profile_.cpuinfo}).discover(info.running_kernel_release);
    ...
    if (!info.root_subvolume.empty() && info.root_subvolume.front() == '/') info.root_subvolume.erase(0, 1);
    return info;
}
