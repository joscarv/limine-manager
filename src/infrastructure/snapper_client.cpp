...
std::vector<std::string> parse_csv_row(const std::string &line) {
    ...
    if (quoted)
        throw std::runtime_error("Malformed CSV output from snapper");
    fields.push_back(std::move(field));
    return fields;
}
...
SnapperConfig SnapperClient::get_config() const {
    ...
    if (result.exit_code != 0)
        throw std::runtime_error("snapper get-config failed: " + result.output);
...
std::vector<SnapshotInfo> SnapperClient::list() const {
    const auto result =
        runner_.run({"snapper", "--config", config_, "--csvout", "--no-headers", "list",
                     "--disable-used-space", "--columns", "number,date,description,read-only"});
    if (result.exit_code != 0)
        throw std::runtime_error("snapper list failed: " + result.output);
