...
std::string title_for(std::string_view pkgbase) {
    ...
    for (const char ch : suffix) {
        ...
        else { title.push_back(upper ? static_cast<char>(std::toupper(static_cast<unsigned char>(ch))) : ch); upper = false; }
    }
    return title;
}

std::vector<std::filesystem::path> microcode_images(const FileSystem& fs, const KernelDiscoveryProfile& profile) {
    ...
    return fs.is_regular_file(path) ? std::vector<std::filesystem::path>{path} : std::vector<std::filesystem::path>{};
}
}
...
std::vector<KernelInstallation> KernelDiscovery::discover(std::string_view running_release) const {
    ...
    if (!filesystem_.is_directory(profile_.boot_mount)) return kernels;
    for (const auto& entry : filesystem_.list_directory(profile_.boot_mount)) {
        if (!entry.regular_file) continue;
        ...
        if (filesystem_.is_regular_file(initramfs)) kernel.initrds.push_back(initramfs);
