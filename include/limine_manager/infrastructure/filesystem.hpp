...
public:
    ...
    [[nodiscard]] virtual std::vector<DirectoryEntry> list_directory(const std::filesystem::path& path) const = 0;
    [[nodiscard]] virtual std::filesystem::path canonical(const std::filesystem::path& path) const = 0;
};
...
public:
    ...
    [[nodiscard]] std::vector<DirectoryEntry> list_directory(const std::filesystem::path& path) const override;
