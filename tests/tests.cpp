...
public:
    void add_directory(std::filesystem::path path) { directories_.insert(normalize(path)); }
    ...
    bool is_regular_file(const std::filesystem::path& path) const override { return files_.contains(normalize(path)); }
    bool is_directory(const std::filesystem::path& path) const override { return directories_.contains(normalize(path)); }
    ...
    std::vector<limine_manager::infrastructure::DirectoryEntry> list_directory(const std::filesystem::path& path) const override {
        const auto parent = normalize(path);
        std::vector<limine_manager::infrastructure::DirectoryEntry> result;
        for (const auto& directory : directories_) {
            if (directory != parent && directory.parent_path() == parent) result.push_back({directory, false, true});
        }
        for (const auto& [file, content] : files_) {
            (void)content;
            if (file.parent_path() == parent) result.push_back({file, true, false});
        }
        return result;
    }
    std::filesystem::path canonical(const std::filesystem::path& path) const override { return normalize(path); }

private:
    static std::filesystem::path normalize(const std::filesystem::path& path) { return path.lexically_normal(); }
...
public:
    ...
    limine_manager::infrastructure::ProcessResult run(const std::vector<std::string>& arguments) const override {
        const auto it = responses_.find(key(arguments));
        if (it == responses_.end()) return {127, "unexpected command"};
        return it->second;
    }
private:
    static std::string key(const std::vector<std::string>& values) {
        std::string result;
        for (const auto& value : values) { result += value; result.push_back('\0'); }
        return result;
    }
    std::map<std::string, limine_manager::infrastructure::ProcessResult> responses_;
};

limine_manager::infrastructure::KernelInstallation kernel(
    std::string pkgbase, std::string release, std::string title, bool running = false) {
    return {std::move(pkgbase), std::move(release), std::move(title),
            "/boot/vmlinuz-linux", {"/boot/intel-ucode.img", "/boot/initramfs-linux.img"}, running};
}
...
void menu_tree_test() {
    ...
    const std::vector<infrastructure::SnapshotInfo> snapshots{{42, "2026-07-01 12:01:33 +0000", "Before upgrade", true}};
    ...
    assert(output.find("module_path: boot():/intel-ucode.img") < output.find("module_path: boot():/initramfs-linux.img"));
}
...
void multiple_kernel_test() {
    ...
    infrastructure::SystemInfo system;
    system.kernels = {
        kernel("linux", "6.18.13-arch1-1", "Linux", true),
        {"linux-lts", "6.12.40-1-lts", "Linux LTS", "/boot/vmlinuz-linux-lts", {"/boot/initramfs-linux-lts.img"}, false}
    };
    system.kernel_cmdline = domain::KernelCommandLine::parse("root=/dev/mapper/cryptroot rootflags=subvol=@ rw");
    application::PreviewService service;
    render::LimineRenderer renderer;
    const auto output = renderer.render(service.build(system, {{7, "2026-07-01", "test", true}}, config::AppConfig{}));
...
void cmdline_model_test() {
    ...
    auto cmdline = KernelCommandLine::parse("quiet root=/dev/mapper/cryptroot option=\"hello world\" rootflags=subvol=@");
    assert(cmdline.value("root") == "/dev/mapper/cryptroot");
    assert(cmdline.value("option") == "hello world");
    auto repeated = KernelCommandLine::parse(
        "rd.luks.name=11111111-1111-1111-1111-111111111111=data "
...
void configurable_preview_test() {
    ...
    infrastructure::SystemInfo system;
    system.kernels = {
        kernel("linux", "6.18", "Linux", true),
        {"linux-lts", "6.12", "Linux LTS", "/boot/vmlinuz-linux-lts", {"/boot/initramfs-linux-lts.img"}, false}
    };
    system.kernel_cmdline = domain::KernelCommandLine::parse("root=/dev/mapper/cryptroot rootflags=subvol=@ rw");
    ...
    application::PreviewService service;
    render::LimineRenderer renderer;
    const auto output = renderer.render(service.build(system,
        {{2, "2026-07-02", "new", true}, {1, "2026-07-01", "old", true}}, cfg));
...
void simulated_system_integration_test() {
    ...
    const auto mount = [&](const std::string& target, const std::string& field, const std::string& value) {
        runner.respond({"findmnt", "--noheadings", "--raw", "--target", target, "--output", field}, value + "\n");
    };
    ...
    const std::vector<infrastructure::SnapshotInfo> snapshots{{42, "2026-07-01", "Before upgrade", true}};
...
void traditional_encrypt_validation_test() {
    ...
    const auto report = validator.validate(
        system,
        infrastructure::SnapperConfig{"root", "/", "btrfs"},
        {{1, "2026-07-13", "test", true}},
        config::AppConfig{});
    assert(report.valid());
}
...
void apply_service_test() {
    ...
    try { (void)service.apply(stale_plan); }
...
void domain_invariant_test() {
    ...
    bool threw = false;
    try { entry.add_child(MenuNode::directory("invalid")); }
...
void fixture_config_test() {
    using namespace limine_manager;
    const auto fixture = std::filesystem::path(LIMINE_MANAGER_SOURCE_DIR) /
                         "tests/fixtures/config/custom.conf";
    infrastructure::RealFileSystem filesystem;
